...
from keras.layers import Dense, Dropout, LSTM, Bidirectional, Input, Flatten, Activation, RepeatVector, Permute, Lambda, BatchNormalization
...
def worker(params,runParmsDic,return_dict):
    ...
    result_recorder.record_results("Lstm_Univar", params, runParmsDic['optimzation_func'], res, c1, c2)
    return
...
def Run(_runParmsDic):
    ...
    # ------- accuracy ------
    result_recorder.create_headers(header="Lstm_Univar-accuracy", parameters=None)
    ...
    gp_result = gp_minimize(func=objective,
                            ...
                            verbose=True)
    result_recorder.record_best_results("Lstm_Univar", gp_result)
    print("Best Results: ", gp_result)

    # ------- ks ------
    result_recorder.create_headers(header="Lstm_Univar-ks", parameters=None)
